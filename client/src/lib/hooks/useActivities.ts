...
export const useActivities = (id?: string) => {
        ...
        select: data => ({
            ...
            pages: data.pages.map((page) => ({
                ...
                items: page.items.map(activity => {
                    const host = activity.atendees.find(x => x.id === activity.hostId);
                    return {
                        ...activity,
                        isHost: currentUser?.id === activity.hostId,
                        isGoing: activity.atendees.some(x => x.id === currentUser?.id),
    ...
    const { data: activity, isLoading: isLoadingActivity } = useQuery({
        ...
        select: data => {
            const host = data.atendees.find(x => x.id === data.hostId);
            return {
                ...data,
                isHost: currentUser?.id === data.hostId,
                isGoing: data.atendees.some(x => x.id === currentUser?.id),
                hostImageUrl: host?.imageUrl
            }
        }
    });
    ...
    const updateAttendance = useMutation({
        ...
        onMutate: async (activityId: string) => {
            ...
            queryClient.setQueryData<Activity>(['activities', activityId], oldActivity => {
                ...
                const isAttending = oldActivity.atendees.some(x => x.id === currentUser.id)

                return {
                    ...oldActivity,
                    isCancelled: isHost ? !oldActivity.isCancelled : oldActivity.isCancelled,
                    atendees: isAttending ? isHost ? oldActivity.atendees : oldActivity.atendees.filter(x => x.id !== currentUser.id)
                        : [...oldActivity.atendees, {
                            id: currentUser.id,
