    ...
    public class UpdateAttendance
    {
        ...
        public class Handler(IUserAccessor userAccessor, AppDbContext context) : IRequestHandler<Command, Result<Unit>>
        {
            public async Task<Result<Unit>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                var attendance = activity.Atendees.FirstOrDefault(x => x.UserId == user.Id);
                var isHost = activity.Atendees.Any(x => x.IsHost && x.UserId == user.Id);

                if (attendance != null)
                {
                    if (isHost) activity.IsCancelled = !activity.IsCancelled;
                    else activity.Atendees.Remove(attendance);
                }
                else
                {
                    activity.Atendees.Add(new Domain.ActivityAtendee
                    {
                        UserId = user.Id,
